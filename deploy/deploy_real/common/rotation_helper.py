...
def quat_wxyz_to_euler_zyx(quat):
    qw = quat[0]
    qx = quat[1]
    qy = quat[2]
    qz = quat[3]
    euler = np.zeros(3)
    euler[0] = np.arctan2(2 * (qw * qx + qy * qz), 1 - 2 * (qx * qx + qy * qy))
    euler[1] = np.arcsin(2 * (qw * qy - qz * qx))
    euler[2] = np.arctan2(2 * (qw * qz + qx * qy), 1 - 2 * (qy * qy + qz * qz))
    return euler
