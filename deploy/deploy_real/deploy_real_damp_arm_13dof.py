...
class Controller(Node):
    def __init__(self, config_file="g1_21dof.yaml") -> None:
        ...
        self.create_subscription(Pose, 'xr/head_pose', self.teleop_head_pose_callback, 10)
        ...
        self.head_z = 1.6  # Initialize to default standing height
        self.teleop_buttons = {
        ...
        self.action_obs = np.zeros(13, dtype=np.float32)  # Store 13-element policy action
    ...
    def run(self):
        ...
        # Height control based on head pose Z position
        # Formula: 0.75 - (1.6 - head_z) = head_z - 0.85
        # Clamped between 0.52 and 0.75
        target_height = 0.75 - (1.6 - self.head_z)
        self.cmd[3] = np.clip(target_height, 0.52, 0.75)
    ...
    def teleop_head_pose_callback(self, msg):
        # Update head Z position for height control
        self.head_z = msg.position.z
        self.head_z = np.clip(self.head_z, 1.2, 1.6)
